...
int main(int argc, char** argv, char** env) {
	...
	if(init && first_fetch && valid_count == 2)
	{
		...
		#ifdef TRACE
		std::cout << "iss.instr.data(): " << std::dec << iss.instr.data() << std::endl;
		#endif
	...
	if(!top->io_memIF_DMem_enable && top->io_memIF_DMem_wrStrobe == 15 && !top->io_memIF_DMem_readWrite)
	{
		known_case = true;
		init = true;
		default_dbus++;
	...
	if(top->io_memIF_DMem_enable && top->io_memIF_DMem_wrStrobe && top->io_memIF_DMem_readWrite)
	{
	
		
		known_case = true;
		
		rtl_mem.store_strobe(top->io_memIF_DMem_wrStrobe, top->io_memIF_DMem_address, top->io_memIF_DMem_writeData);
	...
	{
		known_case = true;
		top->io_memIF_DMem_readData = rtl_mem.load_strobe(top->io_memIF_DMem_wrStrobe, top->io_memIF_DMem_address);
		top->io_memIF_DMem_dataReady = true;
	...
	if(false && !known_case && init)
	{
		...
		std::cout << "IMem_fetchEnable: " << (int)top->io_memIF_IMem_fetchEnable << std::endl;
		std::cout << "IMem_address: " << (int)top->io_memIF_IMem_address  << std::endl;
		std::cout << "DMem_enable: " << (int)top->io_memIF_DMem_enable  << std::endl;
		std::cout << "DMem_address: " << (int)top->io_memIF_DMem_address  << std::endl;
		
		std::cout << "DMem_writeData: " << (int)top->io_memIF_DMem_writeData  << std::endl;
		std::cout << "DMem_wrStrobe: " << (int)top->io_memIF_DMem_wrStrobe  << std::endl;
		std::cout << "DMem_readWrite: " << (int)top->io_memIF_DMem_readWrite  << std::endl;
