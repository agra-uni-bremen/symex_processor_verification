...
// TODO: apparently the RTL core just returns zero (which is consistent with the spec)
...
void ISS::m_set_csr_value(uint32_t addr, uint32_t value) {
    ...
    switch (addr) {
        ...
        case MTVAL_ADDR:
            ...
            return;
        case MIP_ADDR:
            //write(csrs.mip, M_MIP_WRITE_MASK); // match the read-only part of the RTL core
            return; // actually seems like it should be read-only according to the RISC-V spec (and seems to be the behavior of the RTL core as well)
