...
std::string instr_to_string(Instruction instr) { return to_string(instr); }
...
std::string to_string(Compressed::Opcode c_op, Instruction &instr) {
  ...
  case Compressed::C_LW:
    ss << "c.lw"
       << " x" << instr.c_rs2_small() << ", x" << instr.c_rd_small() << ", "
...
std::string to_string(Instruction instr) {
  ...
  auto reg_op = [&](const char *name) {
    ss << name << " x" << instr.rd() << ", x" << instr.rs1() << ", x"
       << instr.rs2();
  };
  ...
  auto store_op = [&](const char *name) {
    ss << name << " x" << instr.rs2() << ", x" << instr.rs1() << ", "
       << instr.S_imm();
  };
  ...
  auto branch_op = [&](const char *name) {
    ss << name << " x" << instr.rs1() << ", x" << instr.rs2() << ", "
       << instr.B_imm();
  };

  auto csr_reg_op = [&](const char *name) {
    ss << name << " x" << instr.rd() << ", x" << instr.rs1() << ", "
       << csr_to_string(instr.csr());
  };

  auto csr_imm_op = [&](const char *name) {
    ss << name << " x" << instr.rd() << ", " << instr.zimm() << ", "
       << csr_to_string(instr.csr());
  };
  ...
  case Opcode::JALR:
    ss << "jalr x" << instr.rd() << ", x" << instr.rs1() << ", "
       << instr.I_imm();
    break;
